...
def loadConf():
	...
	global program_versions_original
	...
	conf_f = open("ginseng_bench.json")
	conf = json.load(conf_f)
	conf_f.close()

	driver = conf["GinsengSettings"]["Driver"]
	
	modified_root = conf["ProgramSettings"]["ModifiedRoot"]
	original_root= conf["ProgramSettings"]["OriginalRoot"]
	program_versions = conf["ProgramSettings"]["DSU_Versions"]
	program_versions_original = conf["ProgramSettings"]["Original_Versions"]
...
def latex_init():
	global latex_terse
	global latex_verbose
	latex_terse = open(results_directory + "latex_terse.tex", "w")
	latex_verbose = open(results_directory + "latex_verbose.tex", "w")
...
def startFTP(log_fname, use_dsu, version):
	...
	Note: It turns out Ginseng debugging (needed for timing info) does not
	work unless the program is executed directly from the directory.
	"""
	log_f = open(log_fname, "a")
	vsftpd_binary = ""
	if use_dsu:
		vsftpd_binary = modified_root + program_versions[version]
	else:
		vsftpd_binary = original_root + program_versions_original[version] + "/vsftpd"
	...
	if use_dsu:
		cwd = os.getcwd()
		os.chdir(modified_root)
		ftp_proc = Popen(args=[vsftpd_binary], stdout=log_f)
		os.chdir(cwd)
	else:		
		ftp_proc = Popen(args=[vsftpd_binary], stdout=log_f)
	return ftp_proc.pid
...
def doUpdate(log_fname, use_dsu, next_version):
	...
	if use_dsu:
		...
		os.system("killall -SIGUSR2 vsftpd-0.exe")
	else:
		# errr, this might never get used; clean up later
		os.kill(vsftpd_pid, 9)
		upd_script = original_root + \
			program_versions_original[next_version] + "/vsftpd"
	...
	if use_dsu:
		...
		print "Updated to " + program_versions_original[next_version]
...
def runUpdateTests(output_fname, time_fname, mem_fname, number_of_runs):
	...
	for i in range(len(program_versions) - 1):			
		...
		#print open(modified_root+"DSU_output.txt").read()
		next_version += 1
		time.sleep(0.1)
...
These results are generated from kitsune/examples/vsftpd/ftp-tests/run\_ginseng\_benchmark.py. \\\\
...
This benchmark measures how long a set of tests takes to run N times on a freshly started version. The idea behind it is to see if regular operations take longer on the Ginseng version than on the original. The time it takes to start vsftpd is not included. \\

\\begin{center}
\\begin{tabular}{ | c | c | c | c | c | }
\\hline
Version & Ginseng Median (s) & Original Median (s) & Diff (Gins - Orig) (s) & Change (\\%%) \\\\
\\hline
...
""" %(program_versions[i], kitsune_cumulative_time, original_cumulative_time,\
		...
		print "Time difference in terms of original vsftpd (median): %.2f" % \
			percent_diff + "%"
...
\\begin{tabular}{ | c | c | c | c | c |}
\\hline
Version & Gins. VmSize (kB) & Orig. VmSize (kB) & Diff (Gins - Orig)  (kB) & Diff ($\\Delta$/Orig) (\\%)\\\\
\\hline
""")
...
\\begin{tabular}{ | c | c | c | c | c | }
\\hline
Version & Gins. VmRSS (kB) & Orig. VmRSS (kB) & Diff (Gins - Orig) (kB) & Diff ($\\Delta$/Orig) (\\%)\\\\
\\hline
""")
...
\\begin{tabular}{ | c | c | c | c | c | }
\\hline
Upd. To Version & Ginseng Median (s) & Original Median (s) & Diff (Gins - Orig) (s) & Change (\\%%) \\\\
\\hline
...
""" %{
	...
	f = open(results_directory + "UPDATE_DATA", "w")
	f.write("--\n")
	f.close()
	open(results_directory + "/UPDATE_STATUS_DATA", "w").close()
...
\\begin{tabular}{ | c | c | c | c | c | }
\\hline
Upd. To Version & Gins. VmSize (kB) & Orig. VmSize (kB) & Diff (Gins. - Orig)  (kB) & Diff ($\\Delta$/Orig) (\\%)\\\\
\\hline
""")
...
\\begin{tabular}{ | c | c | c | c | c | }
\\hline
Upd. To Version & Gins. VmRSS (kB) & Orig. VmRSS (kB) & Diff (Gins - Orig) (kB) & Diff ($\\Delta$/Orig) (\\%)\\\\
\\hline
""")
...
This benchmark times how long it takes for the standalone process to update. This is done by reading timing information from DSU\_output. No connections are made during this test.
...
""")		
	...
	runUpdateTests(OUTPUT_LOG, TIME_LOG_KITSUNE, \
		MEMORY_LOG_KITSUNE, number_of_runs)

	# create regex for searching timing info
	load_patch_re = re.compile(r"LOADPATCH (\d+\.\d+)")
	apply_patch_re = re.compile(r"APPLY_PATCH \d+ (\d+\.\d+)")
	
	f = open(modified_root + "DSU_output.txt", "r")
	contents = f.read()
	f.close()
	lp_data = load_patch_re.findall(contents)
	ap_data = apply_patch_re.findall(contents)
	print lp_data
	print ap_data
	times = ["-"]
	for i in range(len(lp_data)):
		times.append(str((float(lp_data[i]) + float(ap_data[i]))*1000))

	kitsune_mem = parseMemorylogFile("results_ginseng/UPDATE_STATUS_DATA", number_of_runs)
	print kitsune_mem
