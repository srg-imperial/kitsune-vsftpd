...
int
main(int argc, const char* argv[]) E_NOTELOCALS /**DSU data */
{

  /* KITSUNE : print PID and version */
	if (kitsune_is_updating_from("standalone.c") || !kitsune_is_updating()) {
		printf("Version 1.1.1    PID[%i]\n", getpid());
	}

	/* KITSUNE: prevent update blocking */
  ...
  /* Kitsune: lets a demoted process to update */
  ...
  if (tunable_listen && /* Kitsune */
      ( kitsune_is_updating_from("standalone.c") || !kitsune_is_updating())) 
  {
    /* Standalone mode */
  ...
  /* Kitsune: We can skip directly to the client mainloop if we updated from
     there */
  if(kitsune_is_updating_from("postlogin.c")) {
    process_post_login(&the_session);
  } else if(kitsune_is_updating_from("postprivparent.c")) {
    vsf_priv_parent_postlogin(&the_session);
  }	else if(kitsune_is_updating_from("prelogin.c")) {
    init_connection(&the_session);
	} else if(kitsune_is_updating_from("twoprocess.c")) {
    process_login_req(&the_session);
	} else if(kitsune_is_updating()) {
    die("Kitsune main - should not be here");
  } 
	/* End Kitsune */
