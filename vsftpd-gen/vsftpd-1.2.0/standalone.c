...
static struct hash E_G(struct in6_addr, unsigned int) * s_p_ip_count_hash; /**DSU xfgen */
static struct hash E_G(int, struct in6_addr) * s_p_pid_ip_hash; /**DSU xfgen */
...
unsigned int hash_pid(unsigned int buckets, void* p_key);

struct vsf_client_launch
vsf_standalone_main(void) E_NOTELOCALS /**DSU data */
{
	int listen_sock = -1;
	int retval;
	...
	if (tunable_background && 
			!kitsune_is_updating_from("standalone.c")) /* Kitsune update point */
	{
  ...
  if (!kitsune_is_updating()) {
	  s_p_pid_ip_hash = hash_alloc(256, sizeof(int), s_ipaddr_size, hash_pid);
	  s_p_ip_count_hash = hash_alloc(256, s_ipaddr_size, sizeof(unsigned int), hash_ip);
  }

	/* Kitsune: transformers */
	...
	/* Kitsune: don't reinitialize if updating */
	if(!kitsune_is_updating()) {
		if (tunable_listen)
		{
    ...
    kitsune_update("standalone.c");  /**DSU updatepoint */
