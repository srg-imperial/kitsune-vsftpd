...
static struct hash E_G(struct vsf_sysutil_ipv4addr, unsigned int) * s_p_ip_count_hash; /**DSU xfgen */
static struct hash E_G(unsigned int, struct vsf_sysutil_ipv4addr) * s_p_pid_ip_hash; /**DSU xfgen */
...
vsf_standalone_main(void) E_NOTELOCALS /**DSU data */
{
  ...
  while (1)
  {
     ...
     * KITSUNE NOTE - changed to 0 seconds so update happens immediately;
     * this is actually the default in later versions.
     */
