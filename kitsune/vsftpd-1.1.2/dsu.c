...
int LOCAL_XFORM(main, the_session)(void *session) {
	struct vsf_session_old *old_session = (struct vsf_session_old *) 
      stackvars_get_local("main", "the_session");
	assert(old_session);
  ...
  //new_session->p_visited_dir_list = old_session->p_visited_dir_list;
  //*new_session->p_visited_dir_list = the_list;
	...
	if (mystr_l != NULL) {
		...
		new_session->p_visited_dir_list->alloc_len = mystr_l->alloc_len;  		new_session->p_visited_dir_list->p_nodes = (void*) 0;
		int i;	
		for (i = 0; i < num_nodes; i++) {
			str_list_add(new_session->p_visited_dir_list, 
        &(mystr_l->p_nodes[i].str), &(mystr_l->p_nodes[i].sort_key_str));
		}
		str_list_free(mystr_l);
	}*/
	...
	new_session->log_fd = old_session->log_fd;
